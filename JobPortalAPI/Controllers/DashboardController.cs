    ...
    public class DashboardController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ActivityItem>>> GetCandidateActivities()
        {
            try
            {
                ...
                var activityItems = activities.Select(a => new ActivityItem
                {
                    Id = a.ApplicationID,
                    Title = a.Job?.Title,
                    Description = a.Job?.Company != null ? $"Applied to {a.Job.Company.Name}" : "Application submitted",
        ...
        public async Task<ActionResult<IEnumerable<ActivityItem>>> GetRecruiterActivities()
        {
            try
            {
                ...
                var activities = await _context.Applications
                    ...
                    .Where(a => a.Job != null && a.Job.RecruiterID == userId)
                    .OrderByDescending(a => a.AppliedDate)
                    .Take(5)
                    .ToListAsync();
                ...
                {
                    Id = a.ApplicationID,
                    Title = a.Job != null ? $"New application for {a.Job.Title}" : "New application received",
                    Description = a.Candidate?.FullName ?? "Unknown candidate",
                    TimeAgo = GetTimeAgo(a.AppliedDate),
                    Status = a.Status,
                    Icon = "file-earmark-code",
                    CreatedAt = a.AppliedDate
        ...
        public async Task<ActionResult<IEnumerable<JobPerformanceItem>>> GetTopPerformingJobs()
        {
            try
            {
                ...
                // Ensure we have at least 3 items or empty list
                while (topJobs.Count < 3)
                {
                    topJobs.Add(new JobPerformanceItem
                    {
                        Id = 0,
                        Title = null,
                        Location = null,
            ...
            try
            {
                ...
                for (int i = 0; i < 7; i++)
                {
                    ...
                    chartData.ApplicationsData[i] = await _context.Applications
                        .CountAsync(a => a.Job != null && a.Job.RecruiterID == userId &&
